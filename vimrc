...
set formatoptions=qln                                                          " Sets format options when pasting, q= comments can be formated, l= long lines are not broken, n= recognize numbered list when formating test. :h fo-table for reference
...
" Vim fugitive alias
command Gds G --git-dir=$HOME/.cfg/ --work-tree=$HOME status
command Grd G ---git-dir=$HOME/.juanito_rconfig/.cfg/ --work-tree=$HOME/.juanito_rconfig
